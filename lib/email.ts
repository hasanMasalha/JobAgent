...
export async function sendNeedsSecurityCodeEmail(params: {
  userEmail: string;
  userName: string;
  jobTitle: string;
  company: string;
  ...
  const resend = new Resend(process.env.RESEND_API_KEY);
  const { userEmail, userName, jobTitle, company, jobUrl, applicationId } = params;
  const firstName = userName?.split(" ")[0] || "there";
    ...
    <body style="margin:0; padding:0; background:#f9fafb; font-family:Arial,sans-serif;">
      <div style="max-width:560px; margin:40px auto; background:white;
        ...
        <div style="background:#b45309; padding:28px 32px;">
          <div style="color:white; font-size:20px; font-weight:700;">JobAgent</div>
          <div style="color:#fde68a; font-size:13px; margin-top:4px;">Security code needed</div>
        </div>
        ...
        <div style="padding:32px;">
          ...
          <p style="margin:0 0 24px; color:#6b7280; font-size:14px; line-height:1.6;">
            We filled out and submitted your application for <strong style="color:#111827;">${jobTitle}</strong> at
            <strong style="color:#111827;">${company}</strong>, but Greenhouse emailed you a security code to
            verify you're human before it can go through. Check your inbox for that code, then open the link
            below to enter it and finish the application.
          </p>
  ...
  await resend.emails.send({
    from: "JobAgent <notifications@jobagent.uk>",
    to: userEmail,
    subject: `Action needed — enter your security code for ${jobTitle} at ${company}`,
    html,
  });
}

export async function sendNeedsManualEmail(params: {
  userEmail: string;
  userName: string;
  jobTitle: string;
  company: string;
  jobUrl: string;
  applicationId: string;
  cvAttachment?: { filename: string; content: Buffer };
}) {
  const resend = new Resend(process.env.RESEND_API_KEY);
  const { userEmail, userName, jobTitle, company, jobUrl, applicationId, cvAttachment } = params;
  const firstName = userName?.split(" ")[0] || "there";
    ...
    <body style="margin:0; padding:0; background:#f9fafb; font-family:Arial,sans-serif;">
      <div style="max-width:560px; margin:40px auto; background:white;
        ...
        <div style="background:#c2410c; padding:28px 32px;">
          <div style="color:white; font-size:20px; font-weight:700;">JobAgent</div>
          <div style="color:#fed7aa; font-size:13px; margin-top:4px;">Couldn't finish automatically</div>
        </div>
        ...
        <div style="padding:32px;">
          ...
          <p style="margin:0 0 24px; color:#6b7280; font-size:14px; line-height:1.6;">
            We weren't able to complete your application for <strong style="color:#111827;">${jobTitle}</strong> at
            <strong style="color:#111827;">${company}</strong> automatically. Please open the link below and apply
            yourself${cvAttachment ? " — we've attached your tailored CV for this role to save you a step." : "."}
          </p>
          ...
          <table style="width:100%; border-collapse:collapse; margin-bottom:24px;">
            <tr>
              <td style="padding:6px 0; color:#6b7280; font-size:13px;">Status</td>
              <td style="padding:6px 0; text-align:right;">
                <span style="background:#ffedd5; color:#9a3412; padding:2px 10px;
                             border-radius:20px; font-size:12px; font-weight:600;">
                  Needs manual apply
                </span>
              </td>
          ...
          <div style="text-align:center;">
            <a href="${jobUrl}"
               style="display:inline-block; background:#c2410c; color:white;
                      padding:12px 28px; border-radius:8px; text-decoration:none;
                      font-weight:600; font-size:14px;">
              Apply now →
            </a>
